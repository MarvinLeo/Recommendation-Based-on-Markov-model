...
def re_gradient(theta, gre_fun, record, t, n_iter = 10000, convergence = None, alpha = 0.0005, method2 = "logLinear", method1 = "logistic"):
    ...
    if convergence == None:
        for _ in range(n_iter):
            ...
            #x.to_csv(r'Project.txt', header=None, index=None, sep=' ', mode='a')
            if gradient_theta[0] < 0.027:
                break
            gradient_theta = gre_fun(record, theta, t, method2)
    else:
...
#total = len(acu)
#index = 0
thetaResult = np.zeros(len(fileNames))

'''
df = pd.read_csv(fileNames[5])
...
for files in fileNames:
    ...
    record = df.label6[range(df.item_index.size-1100,df.item_index.size-99)]
    record = record.as_matrix()
    thetaResult = re_gradient(theta, sum_gradient, record, 100, alpha = 1)
    index = index + 1
    print(thetaResult)
    #out=pd.DataFrame(thetaResult)
    #out.to_csv(r'Project.txt', header=None, index=None, sep=' ', mode='a')
    break
...
#print(re_gradient(theta, get_gradient_theta, record, 800, alpha = 1))
print(thetaResult[index])

...
import glob
import numpy as np
from experience import get_gamma

class ifmm():
    def __init__(self, rank_num = 10):
        self.N = rank_num;
        self.Theta = np.array([4.09, 1.0, 0.0]);
        #self.acu = np.zeros(len(fileNames))
        #self.rank_list = np.zeros(len(fileNames))
